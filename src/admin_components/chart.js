...
export default function Chart() {
  ...
  const sortedCost = [...TotalCost].sort(
  ...
  const fetchHourly = async (days) => {
    const hours = [];
    const inquiries = [];
    console.log("요청 날짜 : ",days);

    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/analytics/timeseries/hourly?days=${days}`,
    );
    const apiData = res.data;
    console.log("시간대별 사용량 : ", apiData);
  ...
  const fetchDaily = async (days) => {
    ...
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/analytics/daily?start=${formatDate(startDate)}&end=${formatDate(endDate)}`,
    );
    const data = res.data;
    // console.log(data);

    const total_feedback_helpful = data.reduce(
      (sum, item) => sum + (item.feedback_helpful || 0),
      0,
    );
    const total_feedback_not_helpful = data.reduce(
      (sum, item) => sum + (item.feedback_not_helpful || 0),
      0,
    );
    setFeedbackChart({
      labels: ["도움됨", "개선필요"],
    ...
    res.data.forEach((item) => {
      ...
      const score =
        totalFeedback === 0 ? 0 : (item.feedback_helpful / totalFeedback) * 100;
      // console.log(score);
    ...
    const orderedData = [1, 2, 3, 4, 5, 6, 7].map(
      (day) => groupedByWeekday[day] || 0,
    );
  ...
  const fetchCostWindow = (days) => {
    try {
      axios
        .get(`${process.env.REACT_APP_API_URL}/analytics/windows?days=${days}`)
        .then((res) => {
  ...
  const fetchCost = async (days) => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/api-cost/rows?start=${formatDate(startDate)}&end=${formatDate(endDate)}`,
      );
      const data = res.data;
      // console.log(data);
      ...
      const chartgrouped = labels.map((date) => {
        ...
        return {
          ...
          embedding_cost: embeddingData
            ? Number(embeddingData.cost_usd || 0)
            : 0,
        ...
        <div className="chart-main-container">
            ...
            <div className="metrics-row">
              ...
              <div className="metric-card">
                <div className="metric-value success">
                  {APICost?.[3]
                    ? `$ ${APICost[3].total_cost_usd?.toFixed(2)}`
                    : "로딩 중..."}
                </div>
            ...
            <div className="questions-card" style={{ marginBottom: "1.5rem" }}>
                  ...
                  <tbody id="apiUsageTableBody">
                    {currentItems.map((cost) => (
                      <tr key={cost.date}>
                        ...
                        <td>
                          <strong>$ {cost.total_cost.toFixed(2)}</strong>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className="pagination-container">
                <div className="pagination-info">
                  {startIndex + 1} - {Math.min(endIndex, TotalCost.length)} / 총{" "}
                  {TotalCost.length}건
                </div>
            ...
            <div className="info-box">
              ...
              <ul>
                <li>
                  ...
                  ></i>
                  <strong>임베딩 비용:</strong> 지식베이스에 파일 등록 시 문서를
                  벡터화하는 과정에서 발생 (1회성)
                </li>
                <li>
                  <i
                    ...
                    style={{ color: "#28a745", marginRight: "0.5rem" }}
                  ></i>
                  <strong>LLM API 비용:</strong> 챗봇이 검색 결과를 기반으로
                  답변을 생성할 때 발생 (EXAONE 4.0 32B 기준)
                </li>
                <li>
                  <i
                    className="fas fa-check"
                    style={{ color: "#28a745", marginRight: "0.5rem" }}
                  ></i>
                  <strong>음성 API 비용:</strong> 음성 입력을 텍스트로 변환하는
                  비용 (네이버 클로바 STT 기준, 분당 약 $0.047)
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="section" style={{ marginBottom: "2rem" }}>
            ...
            <div className="metrics-row">
              <div className="metric-card">
                <div className="metric-value success">
                  {WindowData.resolve_rate_excluding_noresp}%
                </div>
              ...
              <div className="metric-card">
                <div className="metric-value info">
                  {Number(WindowData.avg_response_ms * 0.001 || 0).toFixed(2)}{" "}
                  초
                </div>
                  ...
                  <div className="question-item" key={index}>
                    ...
                    <div className="question-content">
                      ...
                      <div className="question-count">
                        {faq.views}회 질문 (+{faq.satisfaction_rate}%)
                      </div>
                    </div>
                  </div>
                ))}
